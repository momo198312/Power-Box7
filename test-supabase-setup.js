  ...
  try {
    ...
    if (supabaseAdmin) {
      ...
      const { error: updateError } = await supabaseAdmin
        ...
        .eq("id", (await supabaseAnon.from("hero_section").select("id").limit(1)).data?.[0]?.id);
...
async function main() {
  ...
  if (emptyTables.length > 0) {
    console.log(`⚠️  Empty tables: ${emptyTables.length} (${emptyTables.join(", ")})`);
  }
  
  console.log("\n🔧 NEXT STEPS:");
  
  if (missingTables.length > 0 || emptyTables.length > 0) {
    console.log("1. Run the 'supabase-setup-final.sql' script in your Supabase SQL Editor");
    console.log("   - Go to https://supabase.com/dashboard/project/" + supabaseUrl.split("//")[1].split(".")[0] + "/sql");
    console.log("   - Copy and paste the entire supabase-setup-final.sql content");
  ...
  console.log("- Storage: https://supabase.com/dashboard/project/" + supabaseUrl.split("//")[1].split(".")[0] + "/storage/buckets");
}
