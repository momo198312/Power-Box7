...
export function RealTimeSyncTester() {
  ...
  const startListening = () => {
    ...
    const tables = ['hero_section', 'customer_reviews', 'offer_pricing', 'footer'];
    ...
    const newChannel = supabase
      ...
      .on(
        ...
        (payload) => {
          setIsConnected(true);
          addEvent('hero_section', payload.eventType, `Hero section ${payload.eventType.toLowerCase()}`);
        }
      )
      .on(
        'postgres_changes',
        ...
        (payload) => {
          setIsConnected(true);
          addEvent('customer_reviews', payload.eventType, `Customer reviews ${payload.eventType.toLowerCase()}`);
        }
      )
      .on(
        'postgres_changes',
        ...
        (payload) => {
          setIsConnected(true);
          addEvent('offer_pricing', payload.eventType, `Offer pricing ${payload.eventType.toLowerCase()}`);
        }
      )
      .on(
        'postgres_changes',
        ...
        (payload) => {
          setIsConnected(true);
          addEvent('footer', payload.eventType, `Footer ${payload.eventType.toLowerCase()}`);
        }
      )
      .subscribe((status) => {
  ...
  const triggerTestUpdate = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .eq('id', (await supabase.from('hero_section').select('id').limit(1)).data?.[0]?.id);

      if (error) {
        addEvent('test', 'ERROR', `Test update failed: ${error.message}`);
      } else {
        addEvent('test', 'TRIGGERED', 'Test update sent - waiting for real-time notification');
      }
    } catch (error) {
    ...
    <Card className="w-full max-w-4xl mx-auto">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {events.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-sm">Make changes in the admin panel or trigger a test update</p>
            </div>
          ) : (
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <h4 className="font-medium text-blue-900 mb-2">How to Test Real-time Sync:</h4>
          <ol className="text-sm text-blue-800 space-y-1">
            <li>1. Click "Start Listening" if not already active</li>
            <li>2. Open another browser tab/window to this site</li>
            <li>3. Make changes in the admin panel or click "Trigger Test Update"</li>
